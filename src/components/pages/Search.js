...
const Search = () => {
    ...
    const handleClickEvent = async (evt) => {
        ...
        axios({
            ...
            data: `fields name, platforms, rating, genres, first_release_date, cover.image_id, age_ratings, summary; search "${searchField}"; limit 50;`
        })
        .then(response => {
        ...
        <>
            ...
            <div className="query-container">
                <Row xs={1} sm={1} md={1} lg={2} xl={3} className="g-4 pt-3" key="groups">
                    {games?.map(({ name, platforms, rating, genres, first_release_date, cover, age_ratings, summary }) => (
                        <Col key={cover}>
                        <Card key={cover}className="groups mx-2 mb-5 bg-dark card-container" >
                            ...
                            <Card.Body className=''>
                                <Card.Text>{`Released ${unixConverter(first_release_date)}`}</Card.Text>
                            </Card.Body>
                        </Card>
                    </Col>
                    ))}
